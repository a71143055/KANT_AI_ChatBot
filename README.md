...
주요 구성
- docs/index.html: GitHub Pages에서 제공할 프론트엔드 채팅 UI
- backend/: Claude(Anthropic) 호출을 위한 Express 프록시
...
로컬 테스트
1) 프론트엔드 테스트
   - 브라우저에서 docs/index.html 열기 (백엔드가 로컬에 없으면 실제 API 호출 불가)
...
1) docs/ 파일을 main branch에 병합
2) Settings → Pages → Source: main /docs 선택
3) Pages URL이 발급됩니다
...
4) 배포된 백엔드 URL을 얻은 후, docs/index.html의 BACKEND_URL을 그 URL로 변경하거나 CNAME/리버스 프록시를 사용
